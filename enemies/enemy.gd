...
@export var bounces_when_attacking := false

## Only relevant if bounces_when_walking is true.
@export var walk_bounce_boost := 100.0

## Only relevant if bounces_when_attacking is true.
@export var attack_bounce_boost := 100.0
...
var is_walking_slowly: bool:
	get:
		return _is_behavior_a_slow_walk(current_behavior)
...
var should_face_left: bool:
	get:
		return (
			not is_instance_valid(G.level.player) or
			G.level.player.global_position.x < global_position.x
		)

var player_direction_sign: int:
	get:
		return -1 if should_face_left else 1
...
func _process_behaviors() -> void:
	_process_behavior_transitions()
	_process_behavior_effects()


func _process_behavior_transitions() -> void:
	if (
	...
	match current_behavior:
		...
		Behavior.REPOSITION_FOR_ATTACK:
			...
			if is_player_in_attack_min_range:
				...
				velocity.x = chase_speed * direction_sign
			else:
				_start_behavior(Behavior.ATTACK)
		Behavior.IDLE_PAUSE:
			if is_player_in_detection_range:
	...
	#@export var attack_bounce_boost := 100.0
	#
	# detect edges, prevent walking off, transition behaviors
	#
	# Trigger effects for current behavior:
	# - Movement
	# - Facing direction
	# - Sound
	# - Animation
...
func _start_behavior(
	...
	match current_behavior:
		...
		Behavior.WANDER_LEFT:
			...
			current_behavior_target = global_position + Vector2(-distance, 0)
			var direction_sign := _get_direction_sign_to_target(
		...
		Behavior.REPOSITION_FOR_ATTACK:
			var direction_sign := _get_direction_sign_to_target(
				G.level.player.global_position)
			var target_distance := (
				attack_min_range_radius + G.level.player.half_size.x
			)
			current_behavior_target = Vector2(
...
func _process_behavior_effects() -> void:
	# TODO: Implement behavior movement.
